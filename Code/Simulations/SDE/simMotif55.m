function [RA,RAR,RAinit,RARinit,tRAv,tRARv,RABP,R,BP] = simMotif55(vars,crabp,cypmax,N,dt,stream)
...
for i=2:N
	RAout(i)= RAout(i-1)+ (beta - b*RAout(i-1) + c*RA(i-1))*dt + sigma2*sqrt(dt)*RAout(i-1)*randn(stream);
    RA(i)   = RA(i-1)   + (b*RAout(i-1) - c*RA(i-1) - (alpha*RAR(i-1)/(omega+RAR(i-1)) + gamma*BP(i-1) + eta )*RA(i-1) + delta*RABP(i-1))*dt;
