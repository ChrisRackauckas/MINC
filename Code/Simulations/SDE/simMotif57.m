function [RA,RAR,RAinit,RARinit,tRAv,tRARv,RABP,R,BP] = simMotif57(vars,crabp,cypmax,N,dt,stream)
% RAM5 with additive noise on RA, multiplicative noise on RAout
beta = vars(1);
delta= vars(2);
sigma= vars(3);
eta=   vars(5);
...
for i=2:N
    X(i) = (X(i-1)^4)*dt + (X(i-1)^(5/2))*sqrt(dt)*randn(stream);
	RAout(i)= RAout(i-1)+ (beta - b*RAout(i-1) + c*RA(i-1))*dt;
    RA(i)   = RA(i-1)   + (b*RAout(i-1) - c*RA(i-1) - (alpha*RAR(i-1)/(omega+RAR(i-1)) + gamma*BP(i-1) + eta )*RA(i-1) + delta*RABP(i-1))*dt + sigma*1e-3*sqrt(dt)*X(i);
    ...
    BP(i)   = BP(i-1)   + (a-lambda*BP(i-1)*RAR(i-1) - gamma*BP(i-1)*RA(i-1) + (delta + nu*R(i-1))*RABP(i-1) - u*BP(i-1) + ((d*RAR(i-1))/(e+RAR(i-1))))* + sigma*sqrt(dt)*RAR(i-1)*randn(stream);
end
end