function [RA,RAR,RAinit,RARinit,tRAv,tRARv,RABP,R,BP] = simMotif43(vars,crabp,cypmax,N,dt,stream)
beta = vars(1);
delta= vars(2);
sigma= vars(3);
eta=   vars(5);
...
for i=2:N
    ...
    RA(i)   = RA(i-1)   + (b*RAout(i-1) - c*RA(i-1) - (alpha*RAR(i-1)/(omega+RAR(i-1)) + gamma*BP(i-1) + eta )*RA(i-1) + delta*RABP(i-1))*dt + sigma*sqrt(dt)*RA(i-1)*randn(stream);
    RABP(i) = RABP(i-1) + (gamma*RA(i-1)*BP(i-1) + lambda*RAR(i-1)*BP(i-1) - (delta + nu*R(i-1))*RABP(i-1))*dt + sigma3*sqrt(dt)*RABP(i-1)*randn(stream);
