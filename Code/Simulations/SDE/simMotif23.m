function [RA,RAR,RAinit,RARinit,tRAv,tRARv] = simMotif23(vars,crabp,cypmax,N,dt,stream)
beta = vars(1);
delta= vars(2);
sigma= vars(3);
eta=   vars(5);
...
for i=2:N
    RA(i) = RA(i-1)  + (beta - (alpha*RAR(i-1)/(omega+RAR(i-1)) + gamma + eta )*RA(i-1) + delta*RAR(i-1))*dt + sigma*sqrt(dt)*RA(i-1)*randn(stream);
    RAR(i)= RAR(i-1) +(gamma*RA(i-1) - delta*RAR(i-1))*dt + sigma2*sqrt(dt)*RAR(i-1)*randn(stream);
end
