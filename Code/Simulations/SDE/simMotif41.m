function [RA,RAR,RAinit,RARinit,tRAv,tRARv] = simMotif41(vars,crabp,cypmax,N,dt,stream)
%Motif 4 with multiplicative noise
beta = vars(1);
delta= vars(2);
sigma= vars(3);
eta=   vars(5);
...
for i=2:N
    ...
    RA(i)   = RA(i-1)   + (b*RAout(i-1) - c*RA(i-1) - (alpha*RAR(i-1)/(omega+RAR(i-1)) + gamma*BP(i-1) + eta )*RA(i-1) + delta*RABP(i-1))*dt + sigma*RA(i-1)*sqrt(dt)*randn(stream);
