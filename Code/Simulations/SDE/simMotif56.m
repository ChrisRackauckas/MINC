function [RA,RAR,RAinit,RARinit,tRAv,tRARv,RABP,R,BP] = simMotif56(vars,crabp,cypmax,N,dt,stream)
...
for i=2:N
    ...
    RA(i)   = RA(i-1)   + (b*RAout(i-1) - c*RA(i-1) - (alpha*RAR(i-1)/(omega+RAR(i-1)) + gamma*BP(i-1) + eta )*RA(i-1) + delta*RABP(i-1))*dt + sigma*1e-2*sqrt(dt)*RA(i-1)*randn(stream) + 10*sigma2*sqrt(dt)*randn(stream);
