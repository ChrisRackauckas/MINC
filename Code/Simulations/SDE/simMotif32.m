function [RA,RAR,RAinit,RARinit,tRAv,tRARv,RABP] = simMotif32(vars,crabp,cypmax,N,dt,stream)
beta = vars(1);
delta= vars(2);
sigma= vars(3);
eta=   vars(5);
...
for i=2:N
    RA(i)   = RA(i-1)   + (beta - (alpha*RAR(i-1)/(omega+RAR(i-1)) + gamma + eta )*RA(i-1) + delta*RABP(i-1))*dt + sigma*sqrt(dt)*randn(stream);
