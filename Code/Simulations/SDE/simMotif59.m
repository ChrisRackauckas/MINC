function [RA,RAR,RAinit,RARinit,tRAv,tRARv,RABP,R,BP] = simMotif59(vars,crabp,cypmax,N,dt,stream)
...
for i=2:N
    ...
    RABP(i) = RABP(i-1) + (gamma*RA(i-1)*BP(i-1) + lambda*RAR(i-1)*BP(i-1) - (delta + nu*R(i-1))*RABP(i-1))*dt + sigma*sqrt(dt)*RABP(i-1)*randn(stream);
